//
//  OmnipotentInsuranceSomeReceiveView.m
//  PreserveServerPaid
//
...
//
//万能险部分领取
#import "OmnipotentInsuranceSomeReceiveView.h"
...
@implementation OmnipotentInsuranceSomeReceiveView

{
    UITableView             *tableV;
    NSMutableArray          *chooseArray;
    OmnipotentInsuranceSomeReceiveDetailView   *detailView;
    
}

+(OmnipotentInsuranceSomeReceiveView*)awakeFromNib{
    
    return [[[NSBundle mainBundle] loadNibNamed:@"OmnipotentInsuranceSomeReceiveView" owner:nil options:nil] objectAtIndex:0];
}
-(void)sizeToFit{
    [super sizeToFit];
    [self custemView];
...
-(void)request{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        @try {
            ...
            listBOModel=[remoteService queryUniversalCoverageWithCustomerID:[dic objectForKey:@"customerId"] ];
        }
...
- (IBAction)okBtnClick:(UIButton *)sender {
    ...
    detailView=[[[NSBundle mainBundle] loadNibNamed:@"OmnipotentInsuranceSomeReceiveView" owner:nil options:nil] objectAtIndex:1];
...
@implementation OmnipotentInsuranceSomeReceiveDetailView
{
    UITableView     *tableV;
    ChargeView  *bankView;
...
-(void)request:(NSMutableArray *)code{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        id<TPLListBOModel> listBOModel=nil;
        @try {
            //listBOModel=[remoteService queryUniversalCoverageDetailWithArray:code];
        }
        @catch (NSException *exception) {
        ...
        NSLog(@">>>>>>>>>>>>>%@",listBOModel);
        for (int i=0; i<listBOModel.objList.count; i++) {
            [self.mArray addObject:[[listBOModel.objList objectAtIndex:i] objectForKey:@"accountBOList"]];
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([[listBOModel.errorBean errorCode] isEqualToString:@"1"]) {
                //表示请求出错
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{    
    OmnipotentInsuranceSomeReceiveDetailCellInCell *cell=[tableView dequeueReusableCellWithIdentifier:@"cellInCell"];
    if (!cell) {
        cell=[[OmnipotentInsuranceSomeReceiveDetailCellInCell alloc] initWithFrame:CGRectMake(0, 0, 712, 108)];
        cell.selectionStyle=UITableViewCellSelectionStyleNone;
    }
    NSDictionary *dic=[[self.mArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
    cell.productName.text=[dic objectForKey:@"productName"];
    cell.accountName.text=[dic objectForKey:@"accountName"];
    cell.accunUnits.text=[NSString stringWithFormat:@"%@",[dic objectForKey:@"transferableSoldUnits"]];
    cell.textF.text=[NSString stringWithFormat:@"%@",[dic objectForKey:@"inputAmount"]];
    cell.textF.delegate=self;
    cell.textF.tag=(indexPath.section+1)*100+indexPath.row;
    //accountCode = 11;
//    accountName = "XXX\U8d26\U53f7\U540d\U79f0";
//    accunUnits = 10000;
//    internalId = 2;
//    itemId = 1;
//    policyCode = 15031842745059;
//    productName = "XXXXX\U4fdd\U9669";
//    saleApply = 0;
//    transferableSoldUnits = 0;
    return cell;
}
...
-(void)textFieldDidEndEditing:(UITextField *)textField{
    //先取出cell上对应的输入框
    float num=[[[[self.mArray objectAtIndex:textField.tag/100-1] objectAtIndex:textField.tag%100] objectForKey:@"transferableSoldUnits"] floatValue];
    if (num<[textField.text floatValue]) {
        UIAlertView *alerV=[[UIAlertView alloc] initWithTitle:@"提示信息" message:@"部分领取账户金额不能超过现有账户价值金额" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alerV show];
    }else{
        NSMutableDictionary *dic=[[self.mArray objectAtIndex:textField.tag/100-1] objectAtIndex:textField.tag%100];
        //[[[self.mArray objectAtIndex:textField.tag/100-1] objectAtIndex:textField.tag%100] setObject:textField.text forKey:@"inputAmount"];
        [dic setObject:textField.text forKey:@"inputAmount"];
    }
}
...
- (IBAction)upOrDownBtnClick:(UIButton *)sender {
    if (sender.tag==50) {
        ...
        [[self.redDownView viewWithTag:44] setAlpha:1];
    }else{
        //让箭头向下
        sender.tag=50;
        ...
        [[self.redDownView viewWithTag:44] setAlpha:0];
    }
    bankView.frame=CGRectMake(0, self.redDownView.frame.origin.y-166, 712, 166);
    self.rightView.frame=CGRectMake(0, bankView.frame.origin.y-35, 712, 35);
}
...
@implementation OmnipotentInsuranceSomeReceiveDetailCell
{
    UITableView         *tableV;
}
-(instancetype)initWithFrame:(CGRect)frame{
    self=[[[NSBundle mainBundle] loadNibNamed:@"OmnipotentAdditionalInvestView" owner:nil options:nil] objectAtIndex:2];
    if (self) {
        [self setFrame:frame];
        [self custemView];
    }
...
@implementation OmnipotentInsuranceSomeReceiveDetailCellInCell

-(instancetype)initWithFrame:(CGRect)frame{
    self=[[[NSBundle mainBundle] loadNibNamed:@"OmnipotentInsuranceSomeReceiveView" owner:nil options:nil] objectAtIndex:3];
    if (self) {
        [self setFrame:frame];
    }
    return self;
